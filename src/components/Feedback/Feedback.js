...
const Feedback = ({ values, sendMessage, handleSubmit }) => {
  ...
  const onSubmit = values => new Promise((resolve, reject) => {
    ...
    resolve(sendMessage({ to_email: values.email, to_phone: values.phone, to_comment: values.comments, to_messages: messagesValues}))
  });
    ...
    <Box py={10} id="contact" className="section">
      <Container maxWidth="lg">
        <Grid container alignItems="center">
          <Box component={Grid} pb={5} item xs={12}><Typography variant="h3" component="h3" align="center" data-aos="fade-up" data-aos-duration="1500">Свяжитесь со мной</Typography></Box>
          <Box component={Grid} item xs={12} display="flex" justifyContent="center">
            <form onSubmit={handleSubmit(onSubmit)} className={classes.form}>
              <Grid container spacing={2}>
                <Grid item xs={12}>
                  <TextField name="email" label="Email" fullWidth variant="outlined" validate={email} />
                </Grid>
                <Grid item xs={12}>
                  <TextField name="phone" label="Телефон" fullWidth variant="outlined" validate={required} />
                </Grid>
                <Grid item>
                  <Grid container direction="column">
                    <Grid item><Typography variant="body2">В какой мессенджер вам написать?</Typography></Grid>
                    <Grid item>
                      <CheckBoxGroup 
                        name="messages" 
                        options={[{ label: 'WhatsApp', value: 'WhatsApp', icon: iconWhatsapp}, { label: 'Viber', value: 'Viber', icon: iconViber }, { label: 'Telegram', value: 'Telegram', icon: iconTelegram }]}
                      />
                    </Grid>
                  </Grid>
                </Grid>
